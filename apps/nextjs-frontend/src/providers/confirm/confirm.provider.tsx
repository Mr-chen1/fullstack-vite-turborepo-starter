  ...
  const confirm: ConfirmFunction = useCallback(
    async (options?: ConfirmOptions): Promise<boolean> =>
      new Promise<boolean>((resolve) => {
        ...
        const header = options?.header ?? t('header');
        const message = options?.message ?? t('message');
        const acceptLabel = options?.acceptLabel ?? t('buttons.accept');
        const rejectLabel = options?.rejectLabel ?? t('buttons.reject');

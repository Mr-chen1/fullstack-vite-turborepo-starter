...
6. Migrations (Prisma in `packages/db`): `cd packages/db && npm run migrate:dev` (or `migrate:deploy` for apply-only).
...
| ------------- | --------------------------------------------- | ------------------------------ |
| infra:start   | `docker compose up -d`                        | Start Postgres, Redis, Maildev |
...
| MAIL_PORT           | SMTP port           | number (e.g. 1025 for Maildev)       |

### Frontend `apps/nextjs-frontend/.env`

Source: `apps/nextjs-frontend/.env.example`.

| Variable                    | Purpose           | Format                    |
| --------------------------- | ----------------- | ------------------------- |
| NEXT_PUBLIC_NODE_ENV        | Environment       | development \| production |
| NEXT_PUBLIC_BACKEND_URL     | API base URL      | URL                       |
| NEXT_PUBLIC_MAILDEV_API_URL | Maildev API (dev) | URL                       |

## Docker and Daily Dev

- Infrastructure (Postgres, Redis, Maildev) runs in Docker; apps run on the host via `npm run dev`.
- Optional containerized app builds for CI/CD: see `deploy/` (e.g. `deploy/nextjs-frontend.dockerfile`, `deploy/nestjs-backend.dockerfile`).
